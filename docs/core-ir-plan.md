...
**Status:** Core execution landed (tasks 1, 3, 4, 5 done; 2, 6, 7, 8 open) · **Started:** 2026-07-20 · **Branch:** `interpreter` (PR #23)

Naming drift from the whiteboard sections below, as built: `Scope` → `EnterScope`, `Suspend` →
`HostCall`, `Lam` → `Lambda`, `Prim` → `PrimOp`. The surface AST additionally gained `SafeApply`
(a safe method call `r?.m(args)` is its own node, not `Apply` over `SafeFieldAccess` — the
null-short-circuit semantics must survive to every pass).
...
- [x] **1. Define the core IR and its metadata** — spans everywhere; types/`TypeDef`s/
  ...
  - **Exception found 2026-07-27 — call-before-bind through a `fun` diverges.** The checker's
    ...
    `MachineTest.callBeforeBindThroughAFunctionIsARuntimeError` and end-to-end in
    `BindingEvalTest.readingALaterValThroughAFunctionFailsAtRuntime`; belongs in the
    execution spec when written.
...
- [ ] **2. Checked-program artifact from `check`** — program + type as the stage's output.
  ...
  for its recursive-fun-group rules.) Still open: lowering currently consumes the parsed
  program directly; the host-interface design (docs/ideas/) is what will force this artifact
  into existence, since it must carry the host env for both the lowerer and the link-time
  compatibility check.
- [x] **3. Lowering pass** — checked artifact → IR: slot resolution, erasure, name
  inference. Landed with one semantics bug found and fixed by the eval suites: name
  resolution is **positional** (`LowerEnv` carries a visibility cutoff — a val shadows only
  statements after its own definition, matching the checker; funs/constructors are born
  visible in a hoisted-first slot layout). Host-call tests stay parked until lowering emits
  `HostCall`.
- [x] **4. Pipeline + CLI wiring** — `Klein.lower`/`Klein.execute`; `run`/`r` executes,
  `core` dumps the lowered IR.
- [x] **5. Machine onto the IR** — written fresh over Core rather than ported: a two-stack
  machine (control frames + operand stack, entry-depth collection), frame-carried scopes,
  arm frames for guards with parked-false counting, tail positions as pop-before-push.
  The tree-walker survives as `klein.legacy`, benchmark baseline only, deleted after task 8.
