...
| Destructuring bindings | `{ name, age } = person`, irrefutable-only — see [spec/pattern-matching.md](spec/pattern-matching.md) §9 |
...
| Editions at rest | `encodeEditionJson`/`decodeEditionJson` per [spec/edition.md](spec/edition.md): source, language version, pins, the Core as a versioned binary blob, a checksum over the whole; decoding takes the contract and returns `Intact` or `Stale` with its reason (checksum mismatch, lowerer changed, declaration changed) and never compiles; `compileRule(source, pins)` re-derives. Machine state is never serialized, per the persist-the-log ADR |
