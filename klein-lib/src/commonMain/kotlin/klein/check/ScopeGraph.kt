...
data class ScopeGraph(
    ...
    /**
     * A strongly connected component: its member [nodes] and whether it is [recursive] — either a
     * multi-node cycle or a single node with a self-edge. A non-recursive component is a lone node
     * with no self-reference, so its binding can be typed without its own signature bound first.
     */
    ...
    /**
     * Partition this scope's [nodes] into strongly connected [Component]s (Tarjan's algorithm).
     *
    ...
    fun computeSCCs(): List<Component> {
        ...
        val sccs = mutableListOf<Component>()
        var counter = 0
        ...
        fun strongConnect(v: String) {
            ...
            if (lowlink.getValue(v) == index.getValue(v)) {
                ...
                // A cycle is either more than one node, or a single node that references itself.
                val recursive = component.size > 1 || (v to v) in edges
                sccs.add(Component(component, recursive))
            }
        }
    ...
    companion object {
         ...
         * names are references the checker resolves against an enclosing scope or reports as unbound;
         * [GraphResult.duplicates] are reported as duplicate-binding errors.
         */
        ...
        private fun findReferences(expr: Expr): Pair<List<ScopeGraph>, MutableSet<String>> =
            when (expr) {
                // Nested-scope duplicates are reported during that block's own checking, not
