...
class LexerStatementTest {
    ...
    fun newlineInsideBlock() {
        Lexer(
            ...
            """.trimIndent(),
        ).assertTokens(sym('{'), ident("x"), sym('='), num("1"), stmtEnd, ident("y"), sym('='), num("2"), stmtEnd, sym('}'), eof)
    }
    ...
    fun noNewlineInsideParensInsideBlock() {
        Lexer(
            ...
            """.trimIndent(),
        ).assertTokens(sym('{'), ident("foo"), sym('('), ident("a"), sym(','), ident("b"), sym(')'), stmtEnd, sym('}'), eof)
    }
    ...
    fun newlineInsidePipeLambda() {
        Lexer(
            ...
            |
            """.trimIndent(),
        ).assertTokens(sym('|'), ident("x"), sym('='), num("1"), stmtEnd, ident("y"), stmtEnd, sym('|'), eof)
    }

    @Test
    fun nestedLambdaInParens() {
        Lexer("foo(|.x|)")
            .assertTokens(ident("foo"), sym('('), sym('|'), sym('.'), ident("x"), sym('|'), sym(')'), eof)
    }
    ...
    fun noNewlineAfterComment() {
        Lexer(
            ...
            x = foo(a, # )
                b)
            """.trimIndent(),
        ).assertTokens(ident("x"), sym('='), ident("foo"), sym('('), ident("a"), sym(','), ident("b"), sym(')'), eof)
    }
