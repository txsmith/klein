...
- **[spec/edition.md](./docs/spec/edition.md)** - Editions at rest: the immutable artifact and its contents, the language and compiler versions, the integrity checksum, decoding, re-derivation through pins, migration as a new edition
...
```bash
# Contract alone: prints its environment, declarations and releases, exits non-zero on error
